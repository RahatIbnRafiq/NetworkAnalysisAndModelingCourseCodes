...
for line in f:
    line = line.strip()
    values = line.split(",")
    numberComments.append(values[0])
    posPer.append(values[1])
    posSev.append(values[2])
    negPer.append(values[3])
    negSev.append(values[4])
f.close()

calculateCorrelation(numberComments,posPer)
calculateCorrelation(numberComments,posSev)
calculateCorrelation(numberComments,negPer)
calculateCorrelation(numberComments,negSev)
